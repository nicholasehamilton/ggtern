...
#' ## Use of geom_rug
#' ##-----------------------------------------------
#' #CAUTION: probably should be modified to produce 3 rugs, one next to each axis
#' last_plot() + geom_rug() 
#' 
#' ##-----------------------------------------------
#' ## Use of Combined Geometries
#' ##-----------------------------------------------
#' ggplot(data=DATA.POLY2, aes(x=x,y=y,z=z)) + coord_tern() +
...
#' ##-----------------------------------------------
#' #density2d with polygon and contours,low n for speed,
#' # increase n for better resolution (ie say n = 250)
#' n = 50
#' ggtern(data=DATA.RANDOM,aes(x,y,z)) +
#'   coord_tern(T="y",L="x",R="z") + 
#'   stat_density2d(geom="polygon",aes(fill=..level..,alpha=..level..),n=n) +
#'   geom_density2d(color="red",linetype=2,n=n) + 
#'   geom_point() +
#'   theme_bw()
#' 
#' ##-----------------------------------------------
...
#'               breaks=seq(0.1,1,by=0.1)) + #AFFECT ALL SCALES
#'   theme(axis.tern.arrowsep=unit(0.01,"npc"), #0.01npc away from ticks ticklength
...
#' 
#' ##-----------------------------------------------
#' ## Mahalanobis Confidence Intervals
#' ##-----------------------------------------------
#' plot + geom_confidence(aes(color=..level..),linetype=1,size=1)
#' 
#' ##-----------------------------------------------
#' ## Constant Value Lines
#' ##-----------------------------------------------
#' plot + 
#'   Tline(0.5,color="red",size=1) + 
#'   Lline(0.5,color="blue",size=1) + 
#'   Rline(seq(0.55,0.95,by=0.05),color="green",size=1) 
#' 
#' ##-----------------------------------------------
#' ## No Titles, Ticks, Arrows, Grid or Labels
#' ##-----------------------------------------------
#' plot + 
#'   theme_notitles() + 
#'   theme_noticks()  + 
